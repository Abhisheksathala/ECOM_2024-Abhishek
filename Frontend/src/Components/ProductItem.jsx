...
const ProductItem = ({ id, name, price, image }) => {
  ...
  return (
    <MouseTiltCard tiltIntensity={12} scale={1.03} glareIntensity={0.15}>
      <div
        className="bg-white rounded-xl sm:rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 sm:hover:-translate-y-2 group relative h-full flex flex-col"
        onMouseEnter={() => setIsHovering(true)}
        onMouseLeave={() => setIsHovering(false)}
      >
        <Link to={`/Product/${id}`} className="block h-full flex flex-col">
          <div className="overflow-hidden relative bg-gradient-to-br from-gray-50 to-gray-100 flex-shrink-0">
            <div
          ...
          <div className="p-3 sm:p-4 md:p-5 flex-grow flex flex-col">
            ...
            <div className="flex flex-wrap items-baseline gap-1 sm:gap-2 mb-2 sm:mb-3">
              <p className="text-lg sm:text-xl md:text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">
                {currency}
                {price}
              </p>
